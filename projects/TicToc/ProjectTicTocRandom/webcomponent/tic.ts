...
class EmployeeCard extends HTMLElement{
 ...
 constructor(){
  ...
  document.getElementById("reset-btn")!.onclick = function() {
    alert('reset ');
    //this.initial();
   var  tttArr :any= [null, null, null, null, null, null, null, null, null];
...
function result() {
  ...
  for (var i = 0, len = tttArr.length; i < len; i++) {
    (<HTMLElement>document.getElementsByClassName("grid")[i]).onclick = function() {};
  }
}
function click(index: number) {
  if (tttArr[index] == null) {
    (<HTMLElement>document.getElementsByClassName("grid")[index]).onclick = function() {
...
function comPhase() {
  function isMatchpoint(type: null) {
    if (tttArr[0] == null) {
